...
const crypto_games_providers_url = '/api/v2/casino/crypto/get-providers'
let token = ''

describe('Get Crypto Games Providers List Tests', () => {
    before(() => {
        cy.authorize(login_url, password, user_id, platform_id).then(({body}) => {
            token = body.data.token
        })
    })
    it('Should Return List Of Crypto Games Providers', () => {
        cy.request({
            methtod: 'GET',
            url: crypto_games_providers_url,
            headers: {
