// GET Poker Games Providers List
...
const poker_games_providers_url = '/api/v2/casino/poker/get-providers'
let token = ''

describe('Get Poker Games Providers List Tests', () => {
    before(() => {
        cy.authorize(login_url, password, user_id, platform_id).then(({body}) => {
            token = body.data.token
        })
    })
    it('Should Return List Of Poker Games Providers', () => {
        cy.request({
            methtod: 'GET',
            url: poker_games_providers_url,
            headers: {
